...
public class DifferentCompanyManager {
    ...
    public static String getBaseUrl(CompanyNames companyNames) {
        if (getApplicationComponent().SharedPreferenceModel().checkIsNotEmpty(ADDRESS.getValue())) {
            final String proxy = getApplicationComponent().SharedPreferenceModel().getStringData(ADDRESS.getValue());
        ...
        return switch (companyNames) {
            case ESF -> "https://arzyabi.abfaisfahan.com";
...
//            case ESF -> "http://172.18.12.14:100";
    ...
    public static String getDocumentUrl(CompanyNames companyNames) {
        if (getApplicationComponent().SharedPreferenceModel().checkIsNotEmpty(DOCUMENT_ADDRESS.getValue())) {
            final String proxy = getApplicationComponent().SharedPreferenceModel().getStringData(DOCUMENT_ADDRESS.getValue());
            if (proxy.startsWith("http://") && proxy.length() > 7)
                return proxy;
            if (proxy.startsWith("https://") && proxy.length() > 8)
                return proxy;
...
//            case ESF -> "http://172.18.12.14:100";
    ...
    public static String getMapUrl(CompanyNames companyNames) {
        if (getApplicationComponent().SharedPreferenceModel().checkIsNotEmpty(MAP_ADDRESS.getValue())) {
            final String proxy = getApplicationComponent().SharedPreferenceModel().getStringData(MAP_ADDRESS.getValue());
            if (proxy.startsWith("http://") && proxy.length() > 7)
                return proxy;
            if (proxy.startsWith("https://") && proxy.length() > 8)
                return proxy;
